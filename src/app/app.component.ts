...
export class AppComponent {
  ...
  public tmpRoles: Role[] = [
    ...
    },
    {
      ...
      image: 'innocent',
      description:
        'Vous ne jouez pas la nuit, mais participez au vote du village la journée',
      nb: 0,
      order: 999,
    },
    {
      ...
      image: 'docteur',
      description:
        'Vous pouvez aider un joueur blessé ou donner un médicament douteux à un autre joueur durant la nuit',
    ...
    },
    {
      ...
      image: 'policier',
      description:
        "Vous pouvez empêcher quelqu'un de parler pendant la journée",
    ...
    },
    {
      ...
      image: 'voyant',
      description:
        "Vous pouvez regarder la carte d'un autre joueur toutes les nuits",
    ...
    },
    {
      ...
      image: 'seducteur',
      description:
        'Vous pouvez lier deux personnes qui devront gagner ensemble ou mourir ensemble',
    ...
    },
    {
      ...
      image: 'hommeaffaire',
      description:
        'Vous pouvez user de votre argent pour accélérer drastiquement un vote',
      nb: 0,
      order: 1.8,
      use: 1,
    },
    ...
    },
    {
      ...
      image: 'voleur',
      description:
        'Vous pouvez choisir entre deux rôles tirés aléatoirement au début de la partie',
    ...
    },
    {
      ...
      image: 'switcheur',
      description:
        "Vous pouvez échanger votre carte avec celle d'un autre joueur juste avant le levé du jour",
    ...
    },
    {
      ...
      image: 'trader',
      description:
        'Vous pouvez essayer de deviner qui va mourir la nuit pour tripler vos votes, ou tout perdre si vous vous trompez',
      role: 'Vous pouvez deviner qui va mourir cette nuit pour 3 votes en cas de succès ou 0 en cas de faute ?',
      nb: 0,
      order: 0.1,
    },
  ];
  ...
  public clickJoueur(joueur: Joueur) {
    if (this.joueurSelected == joueur) {
      this.tmpJoueurs.splice(this.tmpJoueurs.indexOf(joueur), 1);
    }
  ...
  public validCartes() {
    ...
    for (let i = 0; i < this.roles.length; i++) {
      ...
      let nb = role.nb;
      for (let j = 0; j < nb; j++) {
        let joueursWoRole = this.joueurs.filter(
          (joueur: Joueur) => !joueur.role
        );
        ...
        if (role.nom == 'Voyant') joueur.voyant = [];
        let tmp = this.roles.find(
          (role: Role) => role.nom == joueur.role?.nom
        )?.nb;
        if (tmp && tmp > 1 && joueur.role?.nom != 'Tueur') {
          joueur.nb =
            '' +
            this.joueurs.filter((j: Joueur) => j.role?.nom == joueur.role?.nom)
              .length;
        }
      }
    }
    ...
    let voleurs = this.joueurs.filter(
      (joueur: Joueur) => joueur.role!.image == 'voleur'
    );
  ...
  public getVolRoles(reste: Role[]) {
    ...
    roles = this.roles.filter(
      (r: Role) =>
        reste.find((rest: any) => rest.nom == r.nom) == undefined &&
        r.nom != 'Innocent' &&
        r.nom != 'Switcheur' &&
        r.image != 'voleur'
    );
  ...
  public nodouble(role: Role) {
    if (role.nom == 'Seducteur') {
      let already = this.joueurs.find(
        (j: any) =>
          j.role.nom == 'Seducteur' || (j.alias && j.alias.nom == 'Seducteur')
      );
      return already == undefined;
    } else if (role.nom == 'CEO') {
      let already = this.joueurs.find(
        (j: any) => j.role.nom == 'CEO' || (j.alias && j.alias.nom == 'CEO')
      );
      return already == undefined;
    }
    return true;
  ...
  public addNbVol(i: number) {
    if (i == -1 && this.nbvol > 1) this.nbvol--;
    else if (i == 1) {
      let nb = this.tmpRoles.filter(
        (r: Role) =>
          r.nom != 'Innocent' && r.nom != 'Switcheur' && r.image != 'voleur'
      ).length;
  ...
  clickNextJoueur() {
    ...
    if (this.joueurSelected.role!.nom == 'Tueur') {
      ...
      if (this.persoClicked) {
        if (this.persoClicked.role?.nom == 'CEO')
          this.persoClicked.role.use = 1;
    ...
    if (this.joueurSelected.role?.order == 999) {
      this.endNuit();
      return;
    }
    this.idx++;
    ...
    if (role) {
      if (this.joueurSelected.defkilled) {
        this.clickNextJoueur();
        return;
      }
      if (role.nom == 'CEO' && role.use == 0) {
        this.clickNextJoueur();
        return;
      }
      if (this.joueurSelected.killed && role.nom != 'Docteur') {
        this.clickNextJoueur();
        return;
      }
      if (role.end) {
        if (role.end <= this.tour) {
          this.clickNextJoueur();
          return;
        }
      }
      if (this.loupPassed && role.nom == 'Tueur') {
        this.clickNextJoueur();
        return;
      }
  ...
  showPlayer(joueurToShow: Joueur) {
    ...
    if (joueurToShow.justProtected && joueurActuel.role?.nom == 'Avocat')
      return false;

    //Le docteur et l'avocat se voient eux mêmes
    if (
      joueurToShow == joueurActuel &&
      (joueurActuel.role?.nom == 'Docteur' ||
        joueurActuel.role?.nom == 'Avocat' ||
        joueurActuel.role?.nom == 'Seducteur')
    )
      return true;
    //Tous les autres joueurs lorsque pas Tueur
    if (joueurToShow != joueurActuel && joueurActuel.role?.nom != 'Tueur')
      return true;
    //Les tueurs ne sont pas dans la liste des tueurs
    if (joueurToShow.role?.nom != 'Tueur' && joueurActuel.role?.nom == 'Tueur')
      return true;
  ...
  checkIfSeducteur() {
    for (let i = 0; i < this.joueurs.length; i++) {
      ...
      if (joueur.killed && joueur.linkedTo) {
        if (!joueur.linkedTo.killed) joueur.linkedTo.killed = true;
      }
    }
  }
  ...
  endVote() {
    ...
    if (this.etape == 'election' && this.president) {
      if (this.unanimite && this.unanimite != '') {
        ...
        for (let i = 0; i < this.joueurs.length; i++) {
          ...
          tmp[i] = {
            joueur: joueur,
            qte: this.joueurs.filter((j: any) => j.select == joueur.nom).length,
          };
        }
        tmp.sort((a, b) => (a.qte > b.qte ? -1 : 1));
        let tabtmp = tmp.filter(
          (a: { joueur: Joueur; qte: number }) => a.qte == tmp[0].qte
        );
    ...
    this.joueurs
      .filter((j: any) => j.killed && !j.defkilled)
      .forEach((j: any) => (j.defkilled = true));
  }
  ...
  checkIfWin() {
    ...
    if (
      joueursRestants.filter((joueur: Joueur) => joueur.role?.nom == 'Tueur')
        .length == 0
    ) {
      this.etape = 'end';
      ...
      if (joueursRestants.length == 1) {
        this.etape = 'end';
        this.winners =
          'Bravo au tueur qui gagne la partie en éliminant tous les innocents !';
        this.badwin.play();
      } else {
        this.etape = 'end';
        this.winners =
          'Bravo aux tueurs qui gagnent la partie en éliminant tous les tueurs !';
        this.badwin.play();
      }
      ...
      if (joueursRestants[0].linkedTo == joueursRestants[1]) {
        this.etape = 'end';
        this.winners =
          'Bravo aux joueurs liés qui gagnent la partie ensemble et contre tous !';
        this.goodwin.play();
      } else if (!this.voteEnded) {
        this.etape = 'end';
        this.winners =
          'Bravo au tueur qui gagne la partie en tuant le dernier joueur avant le début du vote !';
        this.badwin.play();
      }
    }
  }
  ...
  countSelect() {
    ...
    for (let i = 0; i < this.joueurs.length; i++) {
      let joueur = this.joueurs[i];
      if ((!joueur.select || joueur.select == '') && !this.joueurs[i].killed)
        tmp++;
    }
    return tmp;
  }
  ...
  public joueurBlesse() {
    return this.joueurs.find((joueur: Joueur) => joueur.killed && !joueur.out)
      ?.nom;
  }
  ...
  startTimer() {
    this.interval = setInterval(() => {
      ...
      if (this.timer == 0) {
        if (this.etape == 'election' && this.president) {
          let joueursNotDead = this.joueurs.filter(
            (joueur: Joueur) => !joueur.killed
          );
          ...
          let joueursNotDead = this.joueurs.filter(
            (joueur: Joueur) => !joueur.killed && joueur.role?.nom != 'Tueur'
          );
          let rdm = Math.floor(Math.random() * joueursNotDead.length);
          let joueur = joueursNotDead[rdm];
          this.kill(joueur);
  ...
  kill(joueur: Joueur) {
    ...
    if (joueur.role && joueur.role.life == 2 && !joueur.protected)
      joueur.role.life--;
  ...
  getRolesRestants() {
    ...
    for (let i = 0; i < this.roles.length; i++) {
      ...
      roles[i] = {
        role: role,
        qte: tab.filter((r: string) => r == role.nom).length,
      };
    }
